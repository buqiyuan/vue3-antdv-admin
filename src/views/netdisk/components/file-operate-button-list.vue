  ...
  <Space>
    ...
    <Dropdown size="small">
      <Tooltip title="注意：复制或剪切时会覆盖重名文件" placement="top">
        <a-button type="primary" color=" #e6a23c" :disabled="disabledMultiOperateButton">
...
<script lang="ts" setup>
  ...
  const props = defineProps({
    selectedFileList: {
      type: Array as PropType<API.File[]>,
      required: true,
    },
    parsePath: {
      type: Function,
  ...
  const handlePaste = async () => {
    try {
      ...
      if (cutMode.value && !copyMode.value) {
        // cut
        await netdiskManage.fileBatchCut(opData);
        notifyMsg = '剪切';
        cutMode.value = false;
      } else if (!cutMode.value && copyMode.value) {
        // copy
        await netdiskManage.fileBatchCopy(opData);
        notifyMsg = '复制';
        copyMode.value = false;
      } else {
  ...
  const handleDelete = async () => {
    ...
    await netdiskManage.delFileOrDir({
      path,
      files,
    });
